...
for filename in files:
    ...
    with open(out, "a") as output:
        try:
            output.write(filename[:5] + " statistics mean: " + str(truncate(mean(b), tr)) +", std: " + str(truncate(stdev(b),tr)) + "\n")
            print(filename + " statistics mean: " + str(truncate(mean(b),tr)) +", std: " + str(truncate(stdev(b),tr)) )
        except:
            output.write(filename[:5] + " statistics mean: " + str(truncate(mean(b),tr)) +", std: None\n")
